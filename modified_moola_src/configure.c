...
//			tknbase += 5;								//	skip past cache identifier
		...
		if (strcmp(tknbase, "access") == 0) {
			...
			csv_fil_name = valptr;
		} else if (strcmp(tknbase, "-csvfile2") == 0) {
                        csv_fil_name2 = valptr;
...
//			option  = strtol(valptr, NULL, 0);					


		 } else if (strcmp(tknbase, "-cache_flash") == 0) {
                        cache_flash = strtol(valptr, NULL, 0);


		} else if (strcmp(tknbase, "-scheme") == 0) {  // Scheme for the cache
                        scheme = strtol(valptr, NULL, 0);
		
		 } else if (strcmp(tknbase, "-cache_test") == 0) { // Level o cache to study
                        cache_test = strtol(valptr, NULL, 0);

		} else if (strcmp(tknbase, "-dynamic_ways") == 0) { // Level o cache to study
                        dynamic_flash = strtol(valptr, NULL, 0);
